...
from vyro_growth.api.go_live_rehearsal_checklist import (
    GoLiveRehearsalChecklistResponse,
    build_go_live_rehearsal_checklist_response,
)
...
from vyro_growth.api.operator_provider_setup_checklist import (
    build_operator_provider_setup_checklist_response,
)
from vyro_growth.api.operator_rehearsal_outcome_report import (
    build_operator_rehearsal_outcome_report_response,
)
...
from vyro_growth.api.supervised_pilot_go_no_go import (
    SupervisedPilotGoNoGoResponse,
    build_supervised_pilot_go_no_go_response,
)
from vyro_growth.api.supervised_pilot_launch_rehearsal_control_map import (
    SupervisedPilotLaunchRehearsalControlMapResponse,
    build_supervised_pilot_launch_rehearsal_control_map_response,
)
...
@app.get("/internal/supervised-validation-run-packet", tags=["internal"])
def supervised_validation_run_packet(
    db: DbSession,
    response: Response,
    x_internal_api_key: Annotated[str | None, Header()] = None,
    state: Annotated[str | None, Query()] = None,
...
def operator_audit_timeline(
    ...
    active_settings = get_settings()
    _require_internal_key(active_settings, x_internal_api_key)
    return build_operator_audit_timeline_response(
        db,
        active_settings,
...
def operator_release_artifact_manifest(
    db: DbSession,
    x_internal_api_key: Annotated[str | None, Header()] = None,
) -> HTMLResponse:
    active_settings = get_settings()
...
def operator_launch_blockers_plan(
    db: DbSession,
    x_internal_api_key: Annotated[str | None, Header()] = None,
) -> HTMLResponse:
    active_settings = get_settings()
...
def operator_provider_setup_checklist(
    db: DbSession,
    x_internal_api_key: Annotated[str | None, Header()] = None,
) -> HTMLResponse:
    active_settings = get_settings()
...
def operator_rehearsal_outcome_report(
    db: DbSession,
    x_internal_api_key: Annotated[str | None, Header()] = None,
) -> HTMLResponse:
    active_settings = get_settings()
    _require_internal_key(active_settings, x_internal_api_key)
    return build_operator_rehearsal_outcome_report_response(db, active_settings)
...
def operator_supervised_pilot_plan(
    db: DbSession,
    x_internal_api_key: Annotated[str | None, Header()] = None,
) -> HTMLResponse:
    active_settings = get_settings()
    _require_internal_key(active_settings, x_internal_api_key)
    return build_operator_supervised_pilot_plan_response(db, active_settings)
...
def operator_supervised_pilot_candidates(
    db: DbSession,
    x_internal_api_key: Annotated[str | None, Header()] = None,
) -> HTMLResponse:
    active_settings = get_settings()
    _require_internal_key(active_settings, x_internal_api_key)
    return build_operator_supervised_pilot_candidates_response(db, active_settings)
...
def operator_supervised_pilot_go_no_go(
    db: DbSession,
    x_internal_api_key: Annotated[str | None, Header()] = None,
) -> HTMLResponse:
    active_settings = get_settings()
...
def go_live_rehearsal_checklist(
    db: DbSession,
    response: Response,
    x_internal_api_key: Annotated[str | None, Header()] = None,
) -> GoLiveRehearsalChecklistResponse:
...
@app.get("/internal/supervised-pilot-go-no-go", tags=["internal"])
def supervised_pilot_go_no_go(
    db: DbSession,
    response: Response,
    x_internal_api_key: Annotated[str | None, Header()] = None,
) -> SupervisedPilotGoNoGoResponse:
    ...
    _require_internal_key(active_settings, x_internal_api_key)
    response.headers["Cache-Control"] = "no-store"
    return build_supervised_pilot_go_no_go_response(db, active_settings)


@app.get("/internal/supervised-pilot-first-send-preflight", tags=["internal"])
def supervised_pilot_first_send_preflight(
    db: DbSession,
    response: Response,
    x_internal_api_key: Annotated[str | None, Header()] = None,
) -> SupervisedPilotFirstSendPreflightResponse:
    ...
    _require_internal_key(active_settings, x_internal_api_key)
    response.headers["Cache-Control"] = "no-store"
    return build_supervised_pilot_first_send_preflight_response(db, active_settings)


@app.get("/internal/supervised-pilot-launch-rehearsal-control-map", tags=["internal"])
def supervised_pilot_launch_rehearsal_control_map(
    db: DbSession,
    response: Response,
    x_internal_api_key: Annotated[str | None, Header()] = None,
) -> SupervisedPilotLaunchRehearsalControlMapResponse:
