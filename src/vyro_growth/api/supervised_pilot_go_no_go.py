"""Read-only supervised pilot go/no-go JSON export.

Phase 59 exposes existing supervised pilot plan, candidate readiness,
provider setup, rehearsal outcome, launch readiness / go-live index,
review and action-readiness queues, owner approval / settings-request
rollups, and operator halt surfaces as one sanitized go/no-go packet.
It reuses SupervisedPilotGoNoGoService. It never executes, builds,
...
class GoNoGoCountResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class GoNoGoNextActionResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class SupervisedPilotGoNoGoResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    generated_at: datetime
    packet_kind: str = "supervised_pilot_go_no_go"
    purpose: str = "manual_owner_supervised_pilot_go_no_go_review_only"
    overall_status: str
    read_only: bool = True
    ...
    prerequisite_counts_by_status: list[GoNoGoCountResponse] = Field(default_factory=list)
    missing_prerequisite_codes: list[str] = Field(default_factory=list)
    candidate_counts_by_readiness: list[GoNoGoCountResponse] = Field(default_factory=list)
    blocked_reason_counts: list[GoNoGoCountResponse] = Field(default_factory=list)
    ready_for_review_count: int = 0
    blocked_candidate_count: int = 0
    total_candidate_count: int = 0
    review_queue_pending_count: int = 0
    review_queue_decided_count: int = 0
    review_queue_counts_by_artifact_type: list[GoNoGoCountResponse] = Field(
        default_factory=list
    )
    action_readiness_candidate_count: int = 0
    action_readiness_counts_by_status: list[GoNoGoCountResponse] = Field(
        default_factory=list
    )
    action_readiness_counts_by_blocker: list[GoNoGoCountResponse] = Field(
        default_factory=list
    )
    approval_packet_count: int = 0
    approval_packet_counts_by_preflight: list[GoNoGoCountResponse] = Field(
        default_factory=list
    )
    settings_request_count: int = 0
    settings_request_pending_count: int = 0
    settings_request_counts_by_type: list[GoNoGoCountResponse] = Field(
        default_factory=list
    )
    settings_request_counts_by_decision: list[GoNoGoCountResponse] = Field(
        default_factory=list
    )
    go_no_go_gates: list[GoNoGoGateResponse] = Field(default_factory=list)
    ...
    next_actions: list[GoNoGoNextActionResponse] = Field(default_factory=list)


def build_supervised_pilot_go_no_go_response(
    db: Session,
    settings: Settings,
    *,
    service: SupervisedPilotGoNoGoService | None = None,
) -> SupervisedPilotGoNoGoResponse:
    builder = service or SupervisedPilotGoNoGoService()
    return SupervisedPilotGoNoGoResponse.model_validate(
        supervised_pilot_go_no_go_payload(builder.build(db, settings))
    )
