"""Read-only supervised pilot launch rehearsal control map JSON export.

Phase 63 exposes existing halt, outbound, supervised-pilot, approval,
settings, launch-readiness, rehearsal, provider-setup, compliance, and
release surfaces as one sanitized control map. It reuses
SupervisedPilotLaunchRehearsalControlMapService. It never executes,
builds, publishes, deploys, applies settings, lifts halt, enables
outbound, calls providers, spends, or changes live state. This export
is not a script runner, not permission to send, not permission to go
...
class ControlCountResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class ControlNextActionResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class SupervisedPilotLaunchRehearsalControlMapResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    generated_at: datetime
    packet_kind: str = "supervised_pilot_launch_rehearsal_control_map"
    purpose: str = "manual_owner_supervised_pilot_launch_rehearsal_control_map_review_only"
    overall_status: str
    read_only: bool = True
    ...
    rehearsal_control_map_is_not_a_script_runner: bool = True
    control_map_is_not_execution: bool = True
    ...
    control_counts_by_category: list[ControlCountResponse] = Field(default_factory=list)
    control_counts_by_status: list[ControlCountResponse] = Field(default_factory=list)
    ...
    control_nodes: list[ControlNodeResponse] = Field(default_factory=list)
    ...
    next_actions: list[ControlNextActionResponse] = Field(default_factory=list)


def build_supervised_pilot_launch_rehearsal_control_map_response(
    db: Session,
    settings: Settings,
    *,
    ...
    return SupervisedPilotLaunchRehearsalControlMapResponse.model_validate(
        supervised_pilot_launch_rehearsal_control_map_payload(builder.build(db, settings))
    )
