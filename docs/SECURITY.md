...
- CLI `contact-validation-plan` / `contact-validation-report` / `supervised-validation-run-packet` / `final-safety-audit` and JSON `GET /internal/contact-validation/plan` / `GET /internal/contact-validation/report` / `GET /internal/supervised-validation-run-packet` / `GET /internal/final-safety-audit` are read-only. CI/defaults never call Apollo, Hunter, NeverBounce, ZeroBounce, OpenAI, Smartlead, calendar, or voice providers.
