...
- CLI `contact-validation-plan` / `contact-validation-report` and JSON `GET /internal/contact-validation/plan` / `GET /internal/contact-validation/report` are read-only. CI/defaults never call Apollo, Hunter, NeverBounce, ZeroBounce, OpenAI, Smartlead, calendar, or voice providers.
