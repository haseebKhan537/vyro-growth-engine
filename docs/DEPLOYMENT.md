...
7. Run `vyro-growth operator-command-center`, open `GET /internal/operator-dashboard`, `GET /internal/operator-review-queue`, `GET /internal/operator-approval-packets`, `GET /internal/operator-action-readiness`, and `GET /internal/operator-settings-change-requests`, and `vyro-growth launch-readiness`, `vyro-growth settings-change-requests`, `vyro-growth settings-execution-preflight`, and `vyro-growth system-status` and review findings before any manual rollout. Recording a review, approval-packet, or settings-change decision from these surfaces does not execute the artifact, packet, or setting. The readiness queue and settings-execution preflight are dry-run only. See `docs/OPERATOR_HEALTH.md`.
