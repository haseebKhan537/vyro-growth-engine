...
7. Run `vyro-growth operator-command-center`, open `GET /internal/operator-dashboard`, `GET /internal/operator-review-queue`, and `GET /internal/operator-approval-packets`, and `vyro-growth system-status` and review findings before any manual rollout. Recording a review or approval-packet decision from the HTML form does not execute the artifact or packet. See `docs/OPERATOR_HEALTH.md`.
