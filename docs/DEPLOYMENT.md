...
`vyro-growth supervised-pilot-first-send-owner-authorization-packet` is a read-only supervised pilot first-send owner authorization packet over existing first-send preflight, launch rehearsal control map, supervised-pilot go/no-go/plan/candidates, owner approval/settings request rollups, launch blockers, readiness index, rehearsal, provider-setup, compliance, and release surfaces. It does not execute commands, apply settings, lift halt, enable outbound, scrape, select candidates, send, approve, spend, build, publish, or deploy. `GET /internal/supervised-pilot-first-send-owner-authorization-packet` is the same packet as sanitized JSON and is also not approval, not permission to send, not permission to go live, and not an execution surface.
...
`vyro-growth live-provider-setup-checklist` is a read-only owner live-provider setup checklist and credential readiness packet over existing provider setup, launch readiness, supervised validation packet, settings preflight, and compliance binder surfaces. It does not verify credentials, call providers, execute the validation run, grant approval, send, enroll, call, book, spend, publish, deploy, apply settings, or lift halt. `GET /internal/live-provider-setup-checklist` is the same packet as sanitized JSON and is also not permission to run a supervised validation and not an execution surface.

`vyro-growth supervised-validation-run --dry-run` is an owner-approval-required refusal/preflight gate over those same Phase 76/73/71 surfaces. It does not execute the validation run, grant approval, call providers, send, enroll, call, book, spend, publish, deploy, apply settings, or lift halt. `GET /internal/supervised-validation-run` is the same gate as sanitized JSON and is also not permission to run a supervised validation and not an execution surface. The actual 200-practice validation requires separate owner approval before execution.
