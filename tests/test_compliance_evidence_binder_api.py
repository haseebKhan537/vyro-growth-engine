...
BINDER_PATH = "/internal/compliance-evidence-binder"
...
def test_compliance_evidence_binder_open_in_development(
    ...
    response = api_client.get(BINDER_PATH)

    assert response.status_code == 200
    body = response.json()
    assert body["packet_kind"] == "compliance_evidence_binder"
    assert body["purpose"] == "manual_owner_review_only"
    ...
    assert body["binder_is_not_go_live"] is True
    ...
    assert "outbound_and_halt" in body
    assert "live_provider_defaults" in body
    assert "no_execution_side_effects" in body
    assert "phi_secrets_redaction" in body
    assert "consent_phone_boundary" in body
    assert "ci_gates" in body
    assert "operator_audit_timeline" in body
    assert "remaining_manual_owner_checklist" in body
    assert db_session.scalar(select(func.count()).select_from(Activity)) == 0


def test_compliance_evidence_binder_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_compliance_evidence_binder_rejects_invalid_key_and_disallows_post(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_compliance_evidence_binder_api_redacts_secrets_and_stays_read_only(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert body["execution_allowed"] is False
    assert body["go_live_permitted"] is False
    assert body["no_execution_side_effects"]["executed"] == 0
