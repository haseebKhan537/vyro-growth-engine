...
from tests.test_final_safety_audit_service import _assert_no_execution
...
PACKET_PATH = "/internal/final-safety-audit"
...
def test_packet_open_in_development(
    ...
    assert body["kill_switch_outbound_disabled"] is True
    assert body["dual_kill_switch_proof"] is True
    assert "phases" in body
    assert "commands" in body
    assert "routes" in body
    assert "migrations" in body
    assert "live_flag_inventory" in body
    assert "owner_approvals_required" in body
...
def test_packet_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_packet_rejects_invalid_key_and_disallows_post(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_packet_api_redacts_secrets_and_stays_read_only(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
