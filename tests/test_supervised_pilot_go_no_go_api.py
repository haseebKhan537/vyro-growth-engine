...
GO_NO_GO_PATH = "/internal/supervised-pilot-go-no-go"
...
def test_supervised_pilot_go_no_go_open_in_development(
    ...
    response = api_client.get(GO_NO_GO_PATH)
    ...
    assert body["source_candidates_command"] == "supervised-pilot-candidates"
    ...
    assert "go_no_go_gates" in body
    assert "blocked_reason_counts" in body
    assert "prerequisite_counts_by_status" in body
    assert "owner_decision_prerequisites" in body
...
def test_supervised_pilot_go_no_go_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_supervised_pilot_go_no_go_rejects_invalid_key_and_disallows_post(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_supervised_pilot_go_no_go_api_redacts_secrets_and_stays_read_only(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    created = SettingsChangeRequestService().create(
        ...
        idempotency_key="api-supervised-pilot-go-no-go",
    ...
    assert body["supervised_pilot_go_no_go_is_not_go_live"] is True
