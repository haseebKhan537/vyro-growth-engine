...
def test_supervised_pilot_first_send_owner_authorization_packet_does_not_execute_or_call_providers() -> None:
    paths = [
        ...
        Path("tests/test_supervised_pilot_first_send_owner_authorization_packet_service.py"),
        Path("tests/test_supervised_pilot_first_send_owner_authorization_packet_api.py"),
        Path("tests/test_supervised_pilot_first_send_owner_authorization_packet_docs.py"),
    ]
    source = "\n".join(path.read_text(encoding="utf-8") for path in paths).lower()
    assert "httpx" not in source
    assert "subprocess" not in source
...
def test_final_safety_audit_does_not_call_live_or_linkedin_providers() -> None:
    paths = [
