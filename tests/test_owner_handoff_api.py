...
HANDOFF_PATH = "/internal/owner-handoff-packet"
...
def test_owner_handoff_open_in_development(
    ...
    response = api_client.get(HANDOFF_PATH)

    assert response.status_code == 200
    body = response.json()
    assert body["packet_kind"] == "owner_go_live_handoff"
    ...
    assert "settings_change_requests" in body
    assert "settings_execution_preflight" in body
    assert "owner_approval_packets" in body
    assert "approved_action_readiness" in body
    assert "remaining_manual_owner_checklist" in body
    assert db_session.scalar(select(func.count()).select_from(Activity)) == 0


def test_owner_handoff_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_owner_handoff_rejects_invalid_key_and_disallows_post(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_owner_handoff_api_redacts_secrets_and_stays_read_only(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["settings_change_requests"]["request_count"] == 1
    assert body["settings_applied"] is False
    assert body["execution_allowed"] is False
    assert body["go_live_permitted"] is False
    assert body["settings_change_requests"]["requests"][0]["decision_status"] == "approved"
