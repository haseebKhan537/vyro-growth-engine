...
def test_execution_plan_run_open_in_development(
    ...
    response = api_client.post("/internal/execution-plans/run", json={})
...
def test_execution_plan_run_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_execution_plan_run_rejects_invalid_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_execution_plan_run_accepts_valid_key_and_stays_dry_run(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert body["auto_executed"] is False
    assert body["executed_count"] == 0
    assert all(item["executed"] is False for item in body["plans"])
    assert all(item["no_execution"] is True for item in body["plans"])
...
def test_latest_execution_plans_require_auth_and_reuse_run(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    denied = api_client.get("/internal/execution-plans")

    assert created.status_code == 200
    assert latest.status_code == 200
    assert latest.json()["execution_plan_run_id"] == created.json()["execution_plan_run_id"]
    assert denied.status_code == 401
    assert db_session is not None


def test_execution_plan_run_rejects_unknown_artifact_type(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(monkeypatch, Settings(environment="development", internal_api_key=""))
    ...
    response = api_client.post(
        "/internal/execution-plans/run",
        json={"artifact_type": "not_a_real_type"},
    )
