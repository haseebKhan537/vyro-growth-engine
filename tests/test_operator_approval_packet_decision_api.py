...
from vyro_growth.api.operator_approval_packets import (
    ...
    OPERATOR_UI_PACKET_DECISION_SOURCE,
)
from vyro_growth.config import Settings
from vyro_growth.database import get_db
from vyro_growth.domain import ReviewDecisionStatus
...
def _seed_packet(db: Session) -> OwnerApprovalPacket:
    _approve_all_families(db)
    ExecutionPlanningService().generate(db, Settings())
    ApprovalPacketService().generate(db, Settings())
    packet = db.scalars(select(OwnerApprovalPacket)).first()
    assert packet is not None
    return packet


def _decision_url(packet_id: object) -> str:
    return f"{OPERATOR_APPROVAL_PACKETS_PATH}/{packet_id}/decision"


def _counts(db: Session) -> dict[str, int]:
    return {
        "decisions": db.scalar(select(func.count()).select_from(OwnerApprovalPacketDecision)) or 0,
        "activities": db.scalar(select(func.count()).select_from(Activity)) or 0,
        "packets": db.scalar(select(func.count()).select_from(OwnerApprovalPacket)) or 0,
...
def test_valid_packet_decision_submission_records_and_redirects(
    ...
    assert posted.headers["location"].endswith(
        f"{OPERATOR_APPROVAL_PACKETS_PATH}/{packet.id}?decision_recorded=1"
    )
    ...
    assert stored.source == OPERATOR_UI_PACKET_DECISION_SOURCE
    ...
    assert stored.spend_attempted is False
    assert stored.campaign_launched is False
    assert stored.pages_published is False
    assert stored.ads_launched is False
    ...
    assert "Owner approved" in body
    assert read_operator_halt(db_session) is HaltStatus.HALTED
    for marker in EXECUTE_MARKERS:
        assert marker not in body.lower()


def test_invalid_packet_decision_is_generic_and_does_not_write(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_missing_packet_decision_is_not_found(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(monkeypatch, Settings(environment="development", internal_api_key=""))
    ...
    assert "Approval packet not found" in missing.text
    assert invalid.status_code == 404
    assert "Approval packet not found" in invalid.text
...
def test_packet_decision_form_requires_internal_access(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    packet = _seed_packet(db_session)
    url = _decision_url(packet.id)
...
def test_packet_decision_notes_are_sanitized_and_escaped(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_packet_double_submit_and_refresh_are_idempotent(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    stored_rows = db_session.scalars(select(OwnerApprovalPacketDecision)).all()
...
def test_packet_decision_update_still_does_not_execute(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_packet_decision_form_has_no_execution_side_effects(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
