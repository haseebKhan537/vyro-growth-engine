...
RUNBOOK_PATH = "/internal/release-candidate-runbook"
...
def test_release_candidate_runbook_open_in_development(
    ...
    response = api_client.get(RUNBOOK_PATH)

    assert response.status_code == 200
    body = response.json()
    assert body["packet_kind"] == "release_candidate_deployment_runbook"
    ...
    assert "release_candidate_identity" in body
    assert "ci_gates_and_local_verification" in body
    assert "safe_environment_defaults" in body
    assert "operator_halt_and_outbound" in body
    assert "manual_deployment_sequence" in body
    assert "rollback_checklist" in body
    assert "post_deploy_verification" in body
    assert "remaining_manual_owner_checklist" in body
    assert db_session.scalar(select(func.count()).select_from(Activity)) == 0


def test_release_candidate_runbook_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_release_candidate_runbook_rejects_invalid_key_and_disallows_post(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_release_candidate_runbook_api_redacts_secrets_and_stays_read_only(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert body["deployment_allowed"] is False
    assert body["runbook_is_not_deployment"] is True
    assert body["manual_deployment_sequence"]
