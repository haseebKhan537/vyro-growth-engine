...
from tests.test_owner_launch_dossier_service import _assert_no_execution
...
DOSSIER_PATH = "/internal/owner-launch-dossier"
...
def test_owner_launch_dossier_open_in_development(
    ...
    response = api_client.get(DOSSIER_PATH)
    ...
    assert body["source_handoff_command"] == "owner-handoff-packet"
...
def test_owner_launch_dossier_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_owner_launch_dossier_rejects_invalid_key_and_disallows_post(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_owner_launch_dossier_api_redacts_secrets_and_stays_read_only(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    created = SettingsChangeRequestService().create(
        ...
        idempotency_key="api-owner-launch-dossier",
    ...
    assert body["owner_launch_dossier_is_not_go_live"] is True
    assert body["dossier_is_not_permission_to_go_live"] is True
