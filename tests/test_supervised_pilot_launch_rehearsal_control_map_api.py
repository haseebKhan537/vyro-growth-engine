...
from tests.test_supervised_pilot_launch_rehearsal_control_map_service import (
...
CONTROL_MAP_PATH = "/internal/supervised-pilot-launch-rehearsal-control-map"
...
def test_supervised_pilot_launch_rehearsal_control_map_open_in_development(
    ...
    response = api_client.get(CONTROL_MAP_PATH)
...
def test_supervised_pilot_launch_rehearsal_control_map_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_supervised_pilot_launch_rehearsal_control_map_rejects_invalid_key_and_disallows_post(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_supervised_pilot_launch_rehearsal_control_map_api_redacts_secrets_and_stays_read_only(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    created = SettingsChangeRequestService().create(
        ...
        idempotency_key="api-supervised-pilot-launch-rehearsal-control-map",
