...
CANDIDATES_PATH = "/internal/supervised-pilot-candidates"
...
def test_supervised_pilot_candidates_open_in_development(
    ...
    response = api_client.get(CANDIDATES_PATH)
    ...
    assert "candidate_scope" in body
    assert "blocked_counts" in body
    assert "scoring_distribution" in body
    assert "candidate_counts_by_readiness" in body
    assert "kill_switch_outbound_disabled" in body
    ...
    assert body["candidate_scope"]["suggested_max_manually_reviewed_sends"] == 0
    assert db_session.scalar(select(func.count()).select_from(Activity)) == 0


def test_supervised_pilot_candidates_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_supervised_pilot_candidates_rejects_invalid_key_and_disallows_post(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_supervised_pilot_candidates_api_redacts_secrets_and_stays_read_only(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    created = SettingsChangeRequestService().create(
        ...
        idempotency_key="api-supervised-pilot-candidates",
    ...
    assert body["supervised_pilot_candidates_is_not_go_live"] is True
    assert body["export_is_not_permission_to_go_live"] is True
