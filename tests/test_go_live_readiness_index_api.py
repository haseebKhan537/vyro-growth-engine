...
from tests.test_go_live_readiness_index_service import _assert_no_execution
...
INDEX_PATH = "/internal/go-live-readiness-index"
...
def test_go_live_readiness_index_open_in_development(
    ...
    response = api_client.get(INDEX_PATH)
...
def test_go_live_readiness_index_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_go_live_readiness_index_rejects_invalid_key_and_disallows_post(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_go_live_readiness_index_api_redacts_secrets_and_stays_read_only(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert body["index_is_not_permission_to_go_live"] is True
