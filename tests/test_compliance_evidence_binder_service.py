...
def _assert_no_execution(payload: dict[str, object]) -> None:
    ...
    assert payload["packet_kind"] == "compliance_evidence_binder"
    assert payload["purpose"] == "manual_owner_review_only"


def test_empty_binder_is_read_only_without_side_effects(db_session: Session) -> None:
    set_operator_halt(db_session, halted=True, reason="keep-halted")
    settings = _settings()
    before_flags = _flags(settings)
    before = _counts(db_session)

    binder = ComplianceEvidenceBinderService().build(db_session, settings)
    text = format_compliance_evidence_binder(binder, as_json=True)
    markdown = format_compliance_evidence_binder(binder, as_json=False)
    payload = json.loads(text)

    assert binder.overall_status in {"blocked", "warning", "ready_for_owner_review"}
    assert binder.go_live_permitted is False
    assert binder.execution_allowed is False
    assert binder.settings_applied is False
    assert binder.halt_changed is False
    assert binder.owner_approved is False
    assert binder.live_action is False
    assert binder.outbound_enabled is False
    assert binder.binder_is_not_go_live is True
    assert binder.outbound_and_halt.keep_outbound_disabled is True
    assert binder.live_provider_defaults.live_providers_enabled is False
    assert binder.no_execution_side_effects.executed == 0
    assert binder.no_execution_side_effects.execution_allowed is False
    assert binder.consent_phone_boundary.consent_to_call_required is True
    assert binder.consent_phone_boundary.cold_calling_disabled is True
    assert binder.consent_phone_boundary.no_live_dialer is True
    assert binder.ci_gates.smoke_gate.documented is True
    assert binder.ci_gates.deploy_config_gate.documented is True
    assert binder.ci_gates.smoke_gate.job_name == "smoke-dry-run"
    assert binder.ci_gates.deploy_config_gate.job_name == "deploy-config"
    assert binder.operator_audit_timeline.matching_count == 0
    codes = {item.code for item in binder.remaining_manual_owner_checklist}
    ...
    assert "## Outbound disabled and operator halt evidence" in markdown
    assert "## No-live-provider default config evidence" in markdown
    assert "## No-execution side-effect evidence" in markdown
    assert "## PHI, secrets, and redaction evidence" in markdown
    assert "## Consent-based phone-only boundary evidence" in markdown
    assert "## CI dry-run smoke and deploy-config gates" in markdown
...
def test_populated_binder_consolidates_safe_evidence(db_session: Session) -> None:
    set_operator_halt(db_session, halted=True, reason="keep-halted")
    ...
    service.create(
        ...
        idempotency_key="binder-pending",
    ...
    approved = service.create(
        ...
        idempotency_key="binder-approved",
    )
    ...
    assert binder.reused_summaries.settings_preflight_blocked_count >= 1
    assert binder.reused_summaries.settings_preflight_execution_allowed is False
    assert binder.reused_summaries.owner_handoff_go_live_permitted is False
    assert binder.operator_audit_timeline.matching_count >= 1
    assert any(
        item.event_type == "nppes_discovery_completed"
        for item in binder.operator_audit_timeline.entries
    )
    assert SecretName.SMARTLEAD_API_KEY.value in binder.missing_credential_names
    assert binder.phi_secrets_redaction.secret_values_included is False
    assert binder.phi_secrets_redaction.message_bodies_included is False
    assert binder.phi_secrets_redaction.redaction_applied is True
...
def test_binder_is_deterministic_and_idempotent(db_session: Session) -> None:
    set_operator_halt(db_session, halted=True, reason="keep-halted")
    settings = _settings()
    SettingsChangeRequestService().create(
        ...
        request_type=SettingsChangeRequestType.KEEP_OUTBOUND_DISABLED.value,
        requested_setting_names=["OUTBOUND_ENABLED"],
        idempotency_key="binder-idempotent",
    )
    builder = ComplianceEvidenceBinderService()
    before = _counts(db_session)

    one = builder.build(db_session, settings)
    two = builder.build(db_session, settings)
    payload_one = _without_timestamps(binder_payload(one))
    payload_two = _without_timestamps(binder_payload(two))
...
def test_cli_compliance_evidence_binder_json_is_sanitized(
    db_session: Session,
    ...
    code = main(["compliance-evidence-binder", "--json"])
    ...
    assert payload["cli_command"] == "compliance-evidence-binder"
    assert "outbound_and_halt" in payload
    assert "remaining_manual_owner_checklist" in payload
    _assert_no_leakage(output, SECRET_VALUE, DB_SECRET_URL)
    assert read_operator_halt(db_session) is HaltStatus.HALTED


def test_cli_compliance_evidence_binder_markdown_is_sanitized(
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    code = main(["compliance-evidence-binder"])
    output = capsys.readouterr().out
    ...
    assert "binder_is_not_go_live: true" in output
