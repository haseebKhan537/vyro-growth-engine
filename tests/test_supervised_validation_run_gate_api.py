...
from tests.test_supervised_validation_run_gate_service import _assert_no_execution
...
GATE_PATH = "/internal/supervised-validation-run"
...
def test_gate_open_in_development(
    ...
    response = api_client.get(GATE_PATH)
    ...
    assert body["execute_requested"] is False
    assert body["requested_mode"] == "dry_run"
    ...
    assert body["source_live_provider_command"] == "live-provider-setup-checklist"
    assert "gates" in body
    assert "refusal_reason_codes" in body
...
def test_gate_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_gate_rejects_invalid_key_and_disallows_post(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_gate_api_redacts_secrets_and_stays_read_only(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    response = api_client.get(
        GATE_PATH,
        headers={"X-Internal-Api-Key": "internal-secret"},
        params={"state": "TX", "specialty": "Family Medicine", "max_cohort_size": 200},
    )
...
def test_invalid_cohort_size_is_rejected(
    ...
    response = api_client.get(GATE_PATH, params={"max_cohort_size": 201})
