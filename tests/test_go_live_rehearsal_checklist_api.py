...
from tests.test_go_live_rehearsal_checklist_service import _assert_no_execution
...
CHECKLIST_PATH = "/internal/go-live-rehearsal-checklist"
...
def test_go_live_rehearsal_checklist_open_in_development(
    ...
    assert "rehearsal_steps" in body
    assert "rollback_guidance" in body
    assert "expected_safe_assertions" in body
...
def test_go_live_rehearsal_checklist_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_go_live_rehearsal_checklist_rejects_invalid_key_and_disallows_post(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_go_live_rehearsal_checklist_api_redacts_secrets_and_stays_read_only(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    created = SettingsChangeRequestService().create(
        ...
        idempotency_key="api-go-live-rehearsal-checklist",
    ...
    assert body["go_live_rehearsal_checklist_is_not_go_live"] is True
    assert body["rehearsal_is_not_a_script_runner"] is True
