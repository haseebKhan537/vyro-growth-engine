...
from tests.test_supervised_pilot_plan_service import _assert_no_execution
...
PLAN_PATH = "/internal/supervised-pilot-plan"
...
def test_supervised_pilot_plan_open_in_development(
    ...
    assert body["no_deployment"] is True
    ...
    assert body["source_outcome_command"] == "rehearsal-outcome-report"
    assert body["source_rehearsal_command"] == "go-live-rehearsal-checklist"
    ...
    assert body["source_staged_rollout_command"] == "staged-rollout-plan"
    assert body["source_dossier_command"] == "owner-launch-dossier"
    ...
    assert "pilot_scope" in body
    assert "prerequisites" in body
    assert "runbook_steps" in body
    assert "abort_criteria" in body
    assert "safety_assertions" in body
    ...
    assert body["pilot_scope"]["suggested_max_manually_reviewed_sends"] == 0
    assert all(
        step["runnable"] is False and step["executed"] == 0 for step in body["runbook_steps"]
    )
    assert db_session.scalar(select(func.count()).select_from(Activity)) == 0


def test_supervised_pilot_plan_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_supervised_pilot_plan_rejects_invalid_key_and_disallows_post(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_supervised_pilot_plan_api_redacts_secrets_and_stays_read_only(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    created = SettingsChangeRequestService().create(
        ...
        idempotency_key="api-supervised-pilot-plan",
    ...
    assert body["spend_allowed"] is False
    assert body["supervised_pilot_plan_is_not_go_live"] is True
    ...
    assert (
        int(db_session.scalar(select(func.count()).select_from(LiveSettingsChangeRequest)) or 0)
        == before_requests
    )
