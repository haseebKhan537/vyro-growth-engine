...
from tests.test_rehearsal_outcome_report_service import _assert_no_execution
...
REPORT_PATH = "/internal/rehearsal-outcome-report"
...
def test_rehearsal_outcome_report_open_in_development(
    ...
    assert "rehearsal_step_counts_by_status" in body
    assert "rehearsal_step_counts_by_kind" in body
    assert "rehearsal_step_counts_by_required_owner_approval_type" in body
    assert "failed_safe_assertion_keys" in body
...
def test_rehearsal_outcome_report_rejects_non_development_without_key(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_rehearsal_outcome_report_rejects_invalid_key_and_disallows_post(
    api_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_settings(
...
def test_rehearsal_outcome_report_api_redacts_secrets_and_stays_read_only(
    api_client: TestClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    created = SettingsChangeRequestService().create(
        ...
        idempotency_key="api-rehearsal-outcome-report",
    ...
    assert body["rehearsal_outcome_report_is_not_go_live"] is True
    assert body["report_is_not_permission_to_go_live"] is True
